...
def read_lines(filename, start=0, end=None, chunk=4096):
    ...
    with open(filename, 'rb') as f:
        ...
        if start < 0 or end < 0:
            if start >= 0 and end < 0:                   # pylint: disable=R1716
                # Special case [x:-y]
                end_pos, count = seek_backwards(end)
                end = inf
            elif start < 0 and 0 < end < inf:            # pylint: disable=R1716
                # Special case [-x:y]
...
def _tester():
    "Test read_lines() with thousands of permutations to ensure accuracy"
    ...
    src = "zero one two three four five six seven eight nine ten " +\
          "eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen and so on"
    src = src.split()
